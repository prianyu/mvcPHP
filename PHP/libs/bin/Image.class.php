...
// //水印文字
// define('WATER_TEXT', 'www.webname.com');
// //水印文字旋转角度
...
Class Image {
	...
	static public function verify ($length = NULL, $width = NULL, $height = NULL) {
		...
		$name = C('VERIFY_NAME')?C('VERIFY_NAME'):'verify';
		$fn = C('VERIFY_FUNC');
		...
		for ($i = 0; $i < ceil($length/4); $i++) {
			...
			imagestring($verify, 2, mt_rand(0, $width), mt_rand(0, $height), $seed[mt_rand(0, $len)], $color);
		}
		...
		$_SESSION[$name] = $fn(strtolower($code));
		...
		imageline($verify, 0, $h+8, $width, $h, $fontColor);
	 ...
	 * @param  [String] $pos [文字位置]
	 * @param  [String] $save [保存路径|留空则替换原图]
	 * @return [Boolean]
	 */
	Static Public function text ($img, $text = '', $pos=9,$save = NULL) {
		if (!self::checkCondition($img)) return false;
	...
	Static Public function thumb ($img, $width = '', $height = '', $save = NULL) {
		...
		$thumb_path=empty($save)?C('THUMB_PATH'):$save;
		//原图宽、高、类型
		$imgInfo = getimagesize($img);
		$imgW = $imgInfo[0];
		$imgH = $imgInfo[1];
		...
		//保存路径
		$save = self::savePath($thumb_path, $img);
		$fn = 'image' . $type;
		$fn($thumb, $save);
